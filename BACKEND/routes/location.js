...
router.post('/candidates', async (req, res) => {
  try {
    ...
    let baseRadius = isFinite(Number(accuracy)) ? Math.round(Number(accuracy)) : 10000;
    baseRadius = Math.max(1000, baseRadius);
    ...
    let final = selected.slice(0, TARGET);

    // Fallback: if no results found, try broader admin levels
    if (final.length === 0 && adminLevels.length === 3 && adminLevels.includes(6) && adminLevels.includes(7) && adminLevels.includes(8)) {
      if (DEBUG) console.debug('No results with admin levels [6,7,8], trying fallback [4,5,6,7,8]');
      const fallbackAdminLevels = [4,5,6,7,8];
      const fallbackSelected = [];
      const fallbackChosenIds = new Set();

      for (const probe of probes) {
        if (fallbackSelected.length >= TARGET) break;
        ...
        for (let k = 0; ; k++) {
          ...
          const elems = await overpassTalukasGeom(platProbe, plonProbe, Math.round(r), fallbackAdminLevels);
          if (!Array.isArray(elems) || elems.length === 0) continue;
          ...
          if (containedCandidate) {
            if (!fallbackChosenIds.has(containedCandidate.id)) {
              const dist = Math.round(haversine(lat, lon, containedCandidate.lat, containedCandidate.lon) || 0);
              foundForProbe = { id: containedCandidate.id, name: containedCandidate.name, admin_level: containedCandidate.admin_level, tags: containedCandidate.tags, lat: containedCandidate.lat, lon: containedCandidate.lon, distance_m: dist, probe: probe.name, found_radius: Math.round(r) };
              fallbackChosenIds.add(containedCandidate.id);
            }
            break;
          }
        } // end doubling
        ...
        if (!foundForProbe) {
          ...
          let pick = null;
          for (const cand of perProbeNearest) {
            if (!fallbackChosenIds.has(cand.id)) { pick = cand; break; }
          }
          if (!pick && perProbeNearest.length > 0) pick = perProbeNearest[0];
          if (pick) {
            foundForProbe = { ...pick, probe: probe.name, found_radius: null };
            if (foundForProbe.id) fallbackChosenIds.add(foundForProbe.id);
          }
        }

        if (foundForProbe) {
          fallbackSelected.push(foundForProbe);
        }
      } // end probes

      fallbackSelected.sort((a,b) => {
        const a6 = a.admin_level === '6' ? 0 : 1;
        const b6 = b.admin_level === '6' ? 0 : 1;
        if (a6 !== b6) return a6 - b6;
        return (a.distance_m || 0) - (b.distance_m || 0);
      ...
      final = fallbackSelected.slice(0, TARGET);
      if (final.length > 0 && DEBUG) console.debug('Fallback admin levels found', final.length, 'results');
    }

    // Final fallback: if still no results, return a synthetic location based on coordinates
    if (final.length === 0) {
      if (DEBUG) console.debug('No administrative boundaries found, returning synthetic location');
      final = [{
        id: `synthetic-${lat.toFixed(5)}-${lon.toFixed(5)}`,
    ...
    return res.json({
      success: true,
      source: final.some(f => f.synthetic) ? "synthetic_fallback" : "overpass_talukas_8quadrant_faststop",
